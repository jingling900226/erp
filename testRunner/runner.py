...
from testCase.run import runCase
...
def get_report_collect(start_test_time, endtime, starttime):
    ...
    apk_msg = apkBase.apkInfo(PATH('../img/erp.apk'))
...
def runnerCaseApp(l_devices):
    ...
    suite.addTest(TestInterfaceCase.parametrize(runCase, l_devices=l_devices))
    # suite.addTest(TestInterfaceCase.parametrize(testOpen,l_devices=l_devices))
